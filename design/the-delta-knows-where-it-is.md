...
where it used to be objectively.  It now subtracts where the raw delta
said it should be from where the effective delta says it can be, and
adds the variable obtained by subtracting the skipped history from the
operator’s increasingly fragile confidence.  In zdelta system language
this is called reviewability, or the algebraic difference between where
the whole-apply path thinks nothing is wrong and where the partial path
knows exactly why it is.
