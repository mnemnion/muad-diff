...
pub const TextManager = struct {
    ...
    pub fn initText(allocator: Allocator, text: []const u8) !TextManager {
    ...
    pub fn addDelta(tm: *TextManager, zdelta: *ZDelta) !void {
    ...
    pub fn skippedItems(tm: *const TextManager) []const SkippedDeltaOp {
    ...
    pub fn applyNext(tm: *TextManager) !?void {
        const analyzed = (try tm.advanceToMutation()) orelse return null;
    ...
    pub fn skipNext(tm: *TextManager) !?void {
        const analyzed = (try tm.advanceToMutation()) orelse return null;
        if (analyzed.state == .blocked) return error.UnresolvedZDeltaOp;
    ...
    fn captureSkippedOp(tm: *const TextManager, op: DeltaOp) !SkippedDeltaOp {
    ...
    fn rewriteTailAfterSkippedDelete(tm: *TextManager, len: u32) !void {
    ...
    fn rewriteTailAfterSkippedInsert(tm: *TextManager, skipped_len: u32) !void {
        const zdelta = tm.zdelta.?;
        var rebuilt = std.ArrayListUnmanaged(HarmonizedDeltaOp).empty;
        defer rebuilt.deinit(tm.allocator);
        try rebuilt.ensureTotalCapacity(tm.allocator, zdelta.ops.len);
    ...
    fn harmonizeCurrentDelta(tm: *TextManager) !void {
    ...
    fn applySkipHistoryEntry(tm: *TextManager, skipped: *SkippedDeltaOp, skip_index: u32) !void {
        switch (skipped.op) {
    ...
    fn harmonizeSkippedDelete(tm: *TextManager, at: u32, len: u32, skip_index: u32) !void {
        const zdelta = tm.zdelta.?;
        var rebuilt = std.ArrayListUnmanaged(HarmonizedDeltaOp).empty;
        defer rebuilt.deinit(tm.allocator);
    ...
    fn harmonizeSkippedInsert(tm: *TextManager, at: u32, skipped_len: u32, skip_index: u32) !void {
        const zdelta = tm.zdelta.?;
        var rebuilt = std.ArrayListUnmanaged(HarmonizedDeltaOp).empty;
        defer rebuilt.deinit(tm.allocator);
        try rebuilt.ensureTotalCapacity(tm.allocator, zdelta.ops.len + 1);
    ...
    fn equivalentLen(tm: *const TextManager) u32 {
...
fn testTextManager(
    ...
    return TextManager.init(allocator, before, zdelta);
}
...
test "ZDelta TextManager initText empty" {
    const allocator = testing.allocator;
    var tm = try TextManager.initText(allocator, "");
    defer tm.deinit();
...
test "ZDelta TextManager initText rounds slack evenly" {
    const allocator = testing.allocator;
    var tm = try TextManager.initText(allocator, "abcde");
    defer tm.deinit();
...
test "ZDelta TextManager init empty" {
    const allocator = testing.allocator;
    var tm = try testTextManager(allocator, "", "", &.{});
    defer tm.deinit();
...
test "ZDelta TextManager addDelta attaches and recenters" {
    const allocator = testing.allocator;
    var tm = try TextManager.initText(allocator, "abcd");
    defer tm.deinit();
    tm.pivot = 0;
    tm.t_idx = 9;
    tm.z_idx = 9;
...
test "ZDelta TextManager addDelta refuses to replace active delta" {
    const allocator = testing.allocator;
    var tm = try testTextManager(allocator, "abcd", "X", &.{
        .{ .insert = .{ .offset = 0, .len = 1 } },
        .{ .equal = 4 },
    });
    defer tm.deinit();
...
test "ZDelta TextManager addDelta rejects wrong text length" {
    const allocator = testing.allocator;
    var tm = try TextManager.initText(allocator, "ab");
    defer tm.deinit();
...
test "ZDelta TextManager addDelta grows to cover net positive change" {
    const allocator = testing.allocator;
    var tm = try TextManager.initText(allocator, "abc");
    defer tm.deinit();
...
test "ZDelta TextManager init and split lifecycle are equivalent" {
    const allocator = testing.allocator;
    var init_tm = try testTextManager(allocator, "abcd", "XY", &.{
        .{ .insert = .{ .offset = 0, .len = 1 } },
        .{ .equal = 2 },
        .{ .delete = 1 },
        .{ .insert = .{ .offset = 1, .len = 1 } },
    ...
    var split_tm = try TextManager.initText(allocator, "abcd");
...
test "ZDelta TextManager skipNext skips insert and records text" {
    const allocator = testing.allocator;
    var tm = try testTextManager(allocator, "abcd", "XY", &.{
        .{ .insert = .{ .offset = 0, .len = 1 } },
        .{ .equal = 2 },
        .{ .delete = 1 },
        .{ .insert = .{ .offset = 1, .len = 1 } },
...
test "ZDelta TextManager skipNext skips delete and records text" {
    const allocator = testing.allocator;
    var tm = try testTextManager(allocator, "abcd", "Y", &.{
        .{ .equal = 1 },
        .{ .delete = 2 },
        .{ .insert = .{ .offset = 0, .len = 1 } },
        .{ .equal = 1 },
...
test "ZDelta TextManager skipNext returns null when only equals remain" {
    const allocator = testing.allocator;
    var tm = try testTextManager(allocator, "abcd", "", &.{
        .{ .equal = 4 },
    });
    defer tm.deinit();
...
test "ZDelta TextManager equivalentLen accounts for skipped history" {
    ...
    var skipped_insert = try testTextManager(allocator, "abcd", "X", &.{
        .{ .insert = .{ .offset = 0, .len = 1 } },
        .{ .equal = 4 },
    });
    ...
    var skipped_delete = try testTextManager(allocator, "abcd", "", &.{
        .{ .equal = 1 },
        .{ .delete = 2 },
        .{ .equal = 1 },
    });
...
test "ZDelta TextManager plans front and tail slack" {
    ...
    var front = try testTextManager(allocator, "abc", "X", &.{
        .{ .insert = .{ .offset = 0, .len = 1 } },
        .{ .equal = 3 },
    });
    defer front.deinit();
    ...
    var tail = try testTextManager(allocator, "abc", "X", &.{
        .{ .equal = 3 },
        .{ .insert = .{ .offset = 0, .len = 1 } },
    });
    defer tail.deinit();
...
test "ZDelta TextManager rebases with sufficient total slack" {
    const allocator = testing.allocator;
    var tm = try testTextManager(allocator, "abc", "X", &.{
        .{ .insert = .{ .offset = 0, .len = 1 } },
        .{ .equal = 3 },
    });
    defer tm.deinit();
...
test "ZDelta TextManager keeps midpoint fixed across front-heavy edits" {
    const allocator = testing.allocator;
    var tm = try testTextManager(allocator, "abcdefghij", "12345!", &.{
...
test "ZDelta TextManager grows after exhausting planned slack" {
    ...
    try testing.expect(tm.buffer.len > original_buffer_len);
    try testing.expectEqual(@as(u32, TextManager.growth_fudge), tm.budget);
    try expectManagerText("12345abcd!?efghij??", &tm);
}
...
test "ZDelta TextManager budget tracks net edit pressure" {
    const allocator = testing.allocator;
    var tm = try testTextManager(allocator, "abcdef", "", &.{
        .{ .equal = 6 },
    });
    defer tm.deinit();
    ...
    try testing.expectEqual(@as(u32, TextManager.growth_fudge), tm.budget);
    try testing.expectEqual(@as(u32, 0), tm.start);
    try testing.expectEqual(@as(usize, TextManager.growth_fudge), tm.buffer.len - tm.end);
